...
from modules.dataloader import CustomBatchSchedulerSampler
...
class Dataset:
    ...
    def _get_data_loaders(self):
        ...
        if self.args.k_fold:
            pass
        elif self.args.original_dataset_batch_size != 0:
            original_train_dataset = datasets.ImageFolder(
                os.path.join(self.args.original_dataset_path, "train"),
                self.data_transforms["train"])
            ddpm_train_dataset = datasets.ImageFolder(
                os.path.join(self.args.ddpm_dataset_path),
                self.data_transforms["train"])
            val_dataset = datasets.ImageFolder(
                os.path.join(self.dataset_path, "val"),
                self.data_transforms["val"])
            batch_size = self.args.original_batch_size \
                         + self.args.ddpm_batch_size
            train_dataset = ConcatDataset([original_train_dataset, ddpm_train_dataset])
            train_dataloader = torch.utils.data.DataLoader(
                dataset=train_dataset,
                sampler=CustomBatchSchedulerSampler(
                    dataset=train_dataset,
                    batch_size=batch_size,
                    sub_batch_sizes=[self.args.original_batch_size, self.args.ddpm_batch_size]),
                batch_size=batch_size,
                shuffle=False,
            )
